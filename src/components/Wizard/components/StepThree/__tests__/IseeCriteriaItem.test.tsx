...
describe('<IseeCriteriaItem />', (injectedStore?: ReturnType<typeof createStore>) => {
  ...
  it('test on handleSubmit', async () => {
    ...
    const iseeRelationSelect = (await screen.findByTestId(
      'isee-realtion-select'
    )) as HTMLSelectElement;
    const deleteBtn = screen.getByTestId('delete-button-test') as HTMLButtonElement;
    const rankingBtn = await screen.findByTestId('ranking-button-test');

    fireEvent.click(rankingBtn);
    expect(rankingBtn).toBeInTheDocument();

    const menuASC = await screen.findByRole('menuitem', {
      name: 'components.wizard.stepThree.chooseCriteria.form.rankingOrderAsc',
    });
    ...
    expect(handleFieldValueChanged).toHaveBeenCalledWith('GT', 'operator', data.code);
  ...
  it('Test iseeCriteria TextField iseeStartValue/IseeEndValue', async () => {
    ...
    expect(iseeEndValue.value).toBeDefined();
  });
});
