...
import {
  ...
  getStatusLabel,
  RefundItem,
} from './initiativeRefundsMerchants';
...
const InitiativeRefundsTransactions = () => {
  ...
  const getTableData = (initiativeId: string) => {
    ...
    Promise.all([
      ...
      .then(([transactionsRes, merchantDetailRes]) => {
        ...
        const rowsData: Array<TrxItem> = transactionsRes.content.map((r) => mapTransactionDTO(r));
  ...
  const mapRewardBatchToRefundItem = (
    ...
    numberOfTransactionsSuspended:
      dto.numberOfTransactionsSuspended ?? batch?.numberOfTransactionsSuspended ?? 0,
    numberOfTransactionsRejected:
      dto.numberOfTransactionsRejected ?? batch?.numberOfTransactionsRejected ?? 0,
    numberOfTransactionsElaborated:
      dto.numberOfTransactionsElaborated ?? batch?.numberOfTransactionsElaborated ?? 0,
  ...
  const mapTransactionStatus = (status?: RewardBatchTrxStatusEnum) => {
    switch (status) {
      ...
      case RewardBatchTrxStatusEnum.CONSULTABLE:
        return {
          label: t('pages.initiativeMerchantsTransactions.table.consultable'),
          color: 'default',
        };

      case RewardBatchTrxStatusEnum.SUSPENDED:
        return {
          label: t('pages.initiativeMerchantsTransactions.table.suspended'),
          color: 'warning',
        };
  ...
  const handleRefundAction = async (
    ...
    return serviceMap[type](id, batch.id, payload)
      .then((res) => {
        ...
        setAlert({
          text: t(`pages.initiativeMerchantsTransactions.toast.${isSingle}.${type}`),
          isOpen: true,
          severity: 'success',
        });
      })
      ...
      <Box sx={{ width: '100%', pt: 2, px: 2 }}>
        ...
        >
          ...
          {batch.status === 'EVALUATING' ? (
            ...
            ) : role.replace('operator', 'L').toUpperCase() ===
              batch.assigneeLevel.toUpperCase() ? (
              <RoleActionButton
        ...
        >
          ...
          >
            ...
            <Typography
              variant="body2"
              sx={{ gridColumn: 'span 7', fontWeight: 600, wordBreak: 'break-all' }}
            >
              <Tooltip title={iban || '-'}>
                <Box
                  sx={{
          ...
          >
            ...
            <Select
              ...
              renderValue={(selected) => {
                ...
                return (
                  <Tooltip title={label} disableHoverListener={!selected}>
                    <Box
                      sx={{
                        maxWidth: 250,
                        overflow: 'hidden',
                  ...
                  >
                    <Chip
                      ...
                      sx={{
                        ...
                        backgroundColor:
                          mapped.label === t('pages.initiativeMerchantsTransactions.table.toCheck')
                            ? '#C4DCF5'
                            : '',
                        color:
                          mapped.label === t('pages.initiativeMerchantsTransactions.table.toCheck')
            ...
            >
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={{ whiteSpace: { lg: 'nowrap', md: 'none' } }}>
                    {t('pages.initiativeMerchantsTransactions.table.requestedRefund')}
                  </TableCell>
                  <TableCell sx={{ whiteSpace: { lg: 'nowrap', md: 'none' } }}>
                ...
                {rows.map((row) => {
                  ...
                  return (
                    <TableRow key={row.id} hover>
                      ...
                      <TableCell>
                        <Tooltip
                          title={(row.amountCents / 100).toLocaleString('it-IT', {
                            style: 'currency',
                            currency: 'EUR',
                          })}
                        >
                      ...
                      <TableCell>
                        <Chip
                          ...
                          sx={{
                            ...
                            backgroundColor:
                              row.statusLabel ===
                              t('pages.initiativeMerchantsTransactions.table.toCheck')
                                ? '#C4DCF5'
                                : '',
                            color:
                              row.statusLabel ===
        ...
        <RefundReasonModal
          ...
          onConfirm={async (reason) => {
            ...
            await handleRefundAction(reasonModal.type, [...selectedRows], reason).finally(() =>
